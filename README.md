...
개인전과 팀·공동·꼬리잡기·듀얼 규칙의 자유로운 조합을 지원하며 클래스별 스킬,
상태이상, 훈련장, 자기장, 전장 지도와 게임 밸런스 설정을 하나의 플러그인에서 제공합니다.

## 주요 기능

- 듀얼·꼬리잡기·팀·공동을 토글로 섞는 조합형 경기 모드
- 성장 주 모드: 지형 기반 지역, 연결된 금지구역, 레벨·스탯, 일일 몬스터, 전용 장비·시간 한정 이벤트
...
3. GUI에서 듀얼·꼬리잡기·팀·공동 규칙을 필요한 만큼 토글한 뒤 시작합니다.
...
클래식 또는 성장 주 모드를 선택한 뒤 네 규칙을 조합합니다. 클래식에서 아무 토글도 켜지 않으면 개인전이며,
예를 들어 `팀 + 듀얼`, `공동 + 듀얼`, `팀 + 공동 + 꼬리잡기`처럼 함께 사용할 수 있습니다.

| 토글 | 규칙 |
| --- | --- |
| 듀얼 | 서로 다른 클래스 두 개의 무기와 스킬을 함께 사용합니다. |
| 꼬리잡기 | 지정된 상대 팀만 공격할 수 있습니다. 기생충은 등장하지 않습니다. |
| 팀 | 설정된 팀 인원으로 무작위 편성합니다. 아군 피해와 해로운 상태 부여를 차단하고 지도에 아군을 표시합니다. |
| 공동 | 설정된 조 인원으로 짝지어 이동·기본 공격 역할과 핫바·스킬 역할을 나눕니다. 역할은 무작위 또는 고정할 수 있습니다. |

팀/공동 인원수가 참가자 수에 딱 맞지 않거나 팀+공동의 조 크기가 팀 안에 정확히 들어가지 않으면 시작이 거부됩니다.
꼬리잡기와 팀/공동을 함께 사용하면 개인이 아니라 팀별 상대 팀이 배정됩니다. `/cw target`으로 현재 상대를
다시 확인하고 나침반 위치를 갱신할 수 있습니다.

성장은 `/cw start` 화면 상단에서 선택하거나 `/cw growth start [dual] [team] [tail-tag] [cooperative]`로 시작합니다.
레벨·경험치는 경험치 바에 표시됩니다. 레벨업 아이템을 우클릭하면 스탯을 배분하고, **Shift + F**로 보유 장비를 장착합니다.
지역 경계는 지도와 상태별 색상 파티클로 확인하며, 지역 목록은 `/cw growth regions`에서 확인합니다.
운영자는 `/cw growth config`에서 성장 설정을 조정할 수 있습니다. 맵에 따라 지역 생성이나 보행 경로 확보에 실패할 수 있으며,
이 경우 시작을 취소하고 참가자 상태를 복원합니다. 자세한 규칙·설정·검증 항목은 [성장 모드 문서](.idea/local-notes/growth-mode.md)를 참고하세요.
주·부스탯은 피해 성장을, 효과별 지정 스탯은 쿨다운·속도·소환 수 등을 담당합니다. 기본 공격/스킬 성장에는 상한이 없으며,
클래스 전투 성향에 따라 서로 다른 계수를 사용합니다. 설명에는 각 수치 옆에 계수를 붙이고 현재 스탯·장비를 반영한 값을 표시하며, 증가한 숫자만 초록색으로 강조합니다.
자세한 수치는 [클래스별 성장 수치](.idea/local-notes/growth-class-scaling.md)에 정리했습니다.
...
| `/cw keyword [한글명]` | 모두 | 조회 가능한 키워드 목록 또는 지정한 한글 키워드의 효과를 채팅에 표시합니다. |
...
| `modes.cooperative.fixed-roles` | 무작위 역할이 꺼졌을 때 조 안에 반복 배정할 역할 순서 |
