...
- `/cw help [1-2]`: 명령어 도움말
- `/cw abilities [플레이어]`: 현재 배정된 능력 확인
- `/cw target`: 꼬리잡기 표적 확인 및 나침반 갱신
- `/cw assign <플레이어> <능력> [슬롯]`: 선택 단계 또는 진행 중인 게임에서 능력 강제 배정
- `/cw remove <플레이어> <능력|슬롯|all>`: 배정된 능력 제거
- `/cw reload`: 설정 파일 다시 불러오기

능력명 인수에는 자동 완성되는 영문 설정 키, 클래스 이름 또는 게임 내 한글 이름을 사용할 수 있습니다.
꼬리잡기와 꼬리잡기 듀얼에서는 기생충이 등장하지 않으며, 자신에게 배정된 표적만 개인용 강조 파티클로 표시됩니다.
