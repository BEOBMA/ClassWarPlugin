...
abstract class GameClass : EffectApiAccess {
    ...
    lateinit var abilityScope: AbilityScope
        private set
    open val childAbilities: List<GameClass> get() = emptyList()
    open fun isChildActive(child: GameClass): Boolean = true
    open val survivesDeath: Boolean = false
    open fun onSuspend() {}
    open fun onResume() {}
    ...
    fun inject(playerData: PlayerData) {
        ...
        check(!this::abilityScope.isInitialized || abilityScope.playerData === playerData) {
            "An assigned ability cannot be shared between participants"
        }

        this.playerData = playerData
        this.playerStatus = playerData.entityStatus
        this.game = playerData.initGame
        if (!this::abilityScope.isInitialized) {
            abilityScope = AbilityScope(this, playerData)
        }
    }
