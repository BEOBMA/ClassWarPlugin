package org.beobma.classWarPlugin.gameClass.math

import kotlin.random.Random

/** Pure question/answer generation, independent of chat, timers and player rewards. */
internal class MathProblemGenerator(private val random: Random = Random.Default) {
    companion object {
        const val MAX_DIFFICULTY = 10
        private const val PROBLEM_VARIANTS_PER_DIFFICULTY = 12
    }
    fun timeLimitSeconds(level: Int): Int = when (level) {
        in 1..5 -> 15
        6 -> 20
        7 -> 22
        8 -> 25
    ...
    fun generate(level: Int): Pair<String, Int> = when (level) {
    ...
    private fun generateAppliedArithmeticProblem(): Pair<String, Int> = when (random.nextInt(PROBLEM_VARIANTS_PER_DIFFICULTY)) {
        0 -> {
            val percent = listOf(10, 20, 25, 40, 50, 75).random(random)
        ...
        8 -> {
            val discount = listOf(10, 20, 25, 50).random(random)
