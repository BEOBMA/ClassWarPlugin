...
/** 선택된 토글들의 불변 조합이다. */
data class MatchMode(val modifiers: Set<MatchModifier> = emptySet()) {
    ...
    val displayName: String
        get() = if (modifiers.isEmpty()) "<red><bold>클래식</bold></red>" else modifiers
            .sortedBy { it.ordinal }
            .joinToString(" <dark_gray>+</dark_gray> ") { "<gold><bold>${it.displayName}</bold></gold>" }
    val description: String
        get() = if (modifiers.isEmpty()) "<gray>모든 상대와 싸워 마지막 생존자가 됩니다." else modifiers
            .sortedBy { it.ordinal }
            .joinToString(" <dark_gray>/</dark_gray> ") { "<gray>${it.description}" }

    fun toggled(modifier: MatchModifier): MatchMode =
        MatchMode(if (modifier in modifiers) modifiers - modifier else modifiers + modifier)

    fun serialize(): String = modifiers.sortedBy { it.ordinal }.joinToString(",") { it.name }
